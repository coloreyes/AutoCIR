...
def cirr(
    ...
    **kwargs
    ...
    if split == 'test':
        ...
        json.dump(submission,open(f'{dataset_path}/task/{task}/test_submissions_{dataset_name}_loop_{loop}_{get_time()}.json','w'),indent=6)
        json.dump(group_submission,open(f'{dataset_path}/task/{task}/subset_test_submissions_{dataset_name}_loop_{loop}_{get_time()}.json','w'),indent=6)
        ...
        # with open(os.path.join(submissions_folder_path, preload_dict['test'],loop), 'w') as file:
        #     json.dump(submission, file, sort_keys=True)
        # with open(os.path.join(submissions_folder_path, f"subset_{preload_dict['test']}",loop), 'w') as file:
...
def circo(
    ...
    **kwargs
    ...
    if split == 'test':
        ...
        json.dump(queryid_to_retrieved_images,open(f'{dataset_path}/task/{task}/test_submissions_{dataset_name}_loop_{loop}_{get_time()}.json','w'),indent=6)
