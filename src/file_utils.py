...
def read_captions_file(path:str):
    with open(path, 'r', encoding='utf-8') as file:
        reader = csv.reader(file)
        next(reader)
        return {caption[0]: caption[1] for caption in reader}         
...
def init_folder(dataset_path):
    os.makedirs(f'{dataset_path}/results/suggestions', exist_ok=True)
    os.makedirs(f'{dataset_path}/results/modified_captions', exist_ok=True)
    os.makedirs(f'{dataset_path}/results/remodified_captions', exist_ok=True)
